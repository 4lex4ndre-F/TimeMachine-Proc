...
if(isset($_GET['action']) && $_GET['action'] == 'suppression' && !empty($_GET['id_article']) && is_numeric($_GET['id_article']))
{
	...
	$suppression->execute();
	$message .= '<div class="alert alert-success" role="alert" style="margin-top: 20px;">La photo n°' .$id_article . 'a bien été supprimée</div>';
...
// Contrôle sur la disponibilité du reference en BDD si on est dans le cas d'un ajout car lors de la modification, la référence existera toujours.
			...
			*/
			...
			if(!empty($_FILES['photo']['name']))
			{
				...
				//On transforme $extension afin que tous les caractères sooient en minuscule
	...
	if(!$erreur) //équivaut à if($erreur ==false)
		
		{
			...
			{
				$enregistrement =$pdo->prepare("INSERT INTO pictures (title, header, content, date_record, date_picture, country, city, photo) VALUES (:title, :header, :content, now(), :date_picture, :country, :city, :photo)");		
			}
			elseif(isset($_GET['action']) && $_GET['action'] == 'modification')
			{
				$enregistrement = $pdo->prepare("UPDATE photo SET title = :titlr, header= :header, content =:content, date_record =:date_record, date_picture= :date_picture, country =:country, city =:city, photo=:photo WHERE id=:id");
				$id =$_POST['id'];
	    ...
	    <div class="row">
			<div class="col-sm-4 col-sm-offset-4">
				<form method="post" action="" enctype="multipart/form-data">  <!-- on rajoute enctype parce que l'on va demander de joindre une pièce jointe-->
					...
					<!--
					<div class="form-group">
						<label for="keywords">TAGS (mots clefs)</label>
